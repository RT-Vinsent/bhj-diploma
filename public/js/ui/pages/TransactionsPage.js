...
class TransactionsPage {
  ...
  registerEvents() {
    this.element.addEventListener('click', (event) => {
      ...
      if (event.target.closest('.transaction__remove')) {
        console.log(event.target.closest('.transaction__remove').dataset.id)
  ...
  render(options){
    ...
    Account.get(options.account_id.id, ( err, response ) => {
      if (response.success) {
   ...
   * */
  formatDate(date){
    let dataParse = new Date(Date.parse(date));
    let options = {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    ...
    return dataParse.toLocaleString("ru", options).replace(',', ' в');
  }
